...
import static com.example.demo.config.OpenApiConfig.UNAUTHORIZED_ERROR_RESPONSE_SCHEMA;

@RestController
@RequestMapping("/api/v1/digital-documents/library/auth")
public class AuthController {
    ...
    public ResponseEntity<ApiResponseDto<Void>> register(@Valid @RequestBody RegisterRequestDto request) {
        ...
        ResponseEntity<ApiResponseDto<Void>> result = userService.register(request);

        // COUNTER with tags - for successful registrations
        meterRegistry
                .counter("auth.register.outcomes", "success", Objects.requireNonNull(result.getBody()).getMessage().contains("success") ? "true" : "false")
                .increment();
    ...
    @Operation(summary = "User logout", description = "User Logout API")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "401", description = "Unauthorized",
                    content = @Content(schema = @Schema(ref = "#/components/schemas/" + UNAUTHORIZED_ERROR_RESPONSE_SCHEMA))),
    })
